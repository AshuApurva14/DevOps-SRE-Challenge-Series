...
openssl req -x509 -nodes -newkey rsa:2048 -keyout ./certs/local-proxy.key -out ./certs/local-proxy.crt -days 365 -subj "/CN=*.local"


docker run --rm -it -v ./passwd/:/etc/nginx/ httpd:alpine htpasswd -c /etc/nginx/.htpasswd adminuser
